...
class 레벨2_프린터 {
    fun 내풀이(priorities: IntArray, location: Int): Int {
        ...
        while (checkTarget(queue,"target")) {
            var goBack = false
    ...
    private fun checkTarget(queue: LinkedList<Script>, name: String): Boolean{
        for(script in queue){
            if(script.name==name){
                return true
            }
...
//    ArrayDeque사용, forEachIndexed와 filter extension 사용하는 법 확인해 둘것
    fun 다른풀이(priorities: IntArray, location: Int): Int {
