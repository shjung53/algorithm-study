...
class 택배상자 {
    fun solution(order: IntArray): Int {
        ...
        val newOrder = IntArray(order.size)

        for(index in order.indices) {
            newOrder[order[index]-1] = index
        }
        ...
        while(queue.isNotEmpty() || (subContainer.isNotEmpty() && subContainer.peek() == currentOrder)){
            ...
            if(subContainer.isNotEmpty() && subContainer.peek() == currentOrder) {
                subContainer.pop()
                currentOrder++
                answer++
                continue
